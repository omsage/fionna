...
func TestDevice_Reverse(t *testing.T) {
	adbClient, err := NewClient()
	if err != nil {
		t.Fatal(err)
	}
	...
	localPort := 61000
	remotePort := "scrcpy"
	err = devices[0].ReverseLocalAbstract(remotePort, localPort)
	if err != nil {
		t.Fatal(err)
	}

	err = devices[0].ReverseKillLocalAbstract(remotePort)
	if err != nil {
		t.Fatal(err)
	}
}

func TestDevice_ReverseList(t *testing.T) {
	adbClient, err := NewClient()
	if err != nil {
		t.Fatal(err)
	}
	...
	dataList, err := devices[0].ReverseList()
	if err != nil {
		t.Fatal(err)
	}
...
func TestDevice_ReverseKillAll(t *testing.T) {
	adbClient, err := NewClient()
	if err != nil {
		t.Fatal(err)
	}
...
func TestDevice_Forward(t *testing.T) {
	...
	localPort := 61000
	err = devices[0].ForwardLocalAbstract(localPort, "6790")
	if err != nil {
		t.Fatal(err)
	}
...
func TestDevice_ForwardKill(t *testing.T) {
	...
	err = devices[0].ForwardKill(11111)
	if err != nil {
		t.Fatal(err)
	}
}
